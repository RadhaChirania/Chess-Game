...
void main()
{
	...
	setcolor(CYAN);
	outtextxy(50,150,"Designed by:");
	setcolor(6);
	outtextxy(50,180,"Parnika Agarwal BTBTC16185");
	outtextxy(50,230,"Radha Chirania BTBTC16053");
	outtextxy(50,280,"Saloni Jain BTBTC16305");
	getch();
	cleardevice();
	cleardevice();
...
/////pawn/////
     ...
     if(abs(board[y][x])>=9 &&
abs(board[y][x])<=16){	//if piece selscted has value greater than 8and less than 17
				b = pawn(x,y,x1,y1);
				}//end pawn
...
////queen///
		      ...
		      else if(abs(board[y][x])==4){	////if pieceselscted has value 4
				b = queen(x,y,x1,y1);
				}//end queen
///king///

		      else if(abs(board[y][x])==5){	//if pieceselscted has value 5
				b = king(x,y,x1,y1);
			 }//end king
